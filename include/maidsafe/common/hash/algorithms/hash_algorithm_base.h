...
class HashAlgorithmBase {
 ...
 private:
  ...
  using Normalize = typename std::remove_reference<typename std::remove_cv<Type>::type>::type;

  template <typename Type>
  using IsVersionIntegral =
      std::integral_constant<bool, std::is_integral<typename HashVersion<Type>::value_type>::value>;
  ...
  struct HashAppendFunction {
    ...
    template <typename Type>
    EnableIfHas<MemberFunction(Derived&, Type)> operator()(Derived& hash, Type&& type) const {
      MemberFunction{}(hash, std::forward<Type>(type));
    }

    template <typename Type>
    EnableIfHas<NonMemberFunction(Derived&, Type)> operator()(Derived& hash, Type&& type) const {
  ...
  void InvokeHashAppend(Arg&& param, std::false_type) {
    static_assert(Has<SerializeFunction(Derived&, Arg)>::value,
  ...
  void InvokeSerialize(Arg&& param, std::true_type) {
    static_assert(UseSerializeForHashing<typename std::remove_reference<Arg>::type>::value,
