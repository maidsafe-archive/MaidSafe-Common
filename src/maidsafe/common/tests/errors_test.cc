...
TEST(ErrorsTest, BEH_ErrorCodeErrorCondition) {
  ...
  EXPECT_TRUE(
      GetCommonCategory().equivalent(null_pointer_error.code(), null_pointer_condition.value()));
  EXPECT_FALSE(
      GetCommonCategory().equivalent(null_pointer_error.code().value(), null_pointer_condition));
  EXPECT_FALSE(
      GetCommonCategory().equivalent(data_empty_error.code(), null_pointer_condition.value()));
  EXPECT_FALSE(
      GetCommonCategory().equivalent(data_empty_error.code().value(), null_pointer_condition));
}
...
template <>
struct EnumClass<common_error> {
  typedef CommonErrors type;
};

template <>
struct EnumClass<asymm_error> {
  typedef AsymmErrors type;
};

template <>
struct EnumClass<passport_error> {
  typedef PassportErrors type;
};
...
template <>
struct EnumClass<routing_error> {
  typedef RoutingErrors type;
};

template <>
struct EnumClass<nfs_error> {
  typedef NfsErrors type;
};

template <>
struct EnumClass<drive_error> {
  typedef DriveErrors type;
};

template <>
struct EnumClass<vault_error> {
  typedef VaultErrors type;
};

template <>
struct EnumClass<vault_manager_error> {
  typedef VaultManagerErrors type;
};

template <>
struct EnumClass<api_error> {
