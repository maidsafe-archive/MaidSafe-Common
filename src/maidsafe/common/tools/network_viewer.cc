/*  Copyright 2013 MaidSafe.net limited
    ...
    This MaidSafe Software is licensed to you under (1) the MaidSafe.net
   Commercial License,
    version 1.0 or later, or (2) The General Public License (GPL), version 3,
   depending on which
    licence you accepted on initial access to the Software (the "Licences").

    By contributing code to the MaidSafe Software, or to this project generally,
   you agree to be
    bound by the terms of the MaidSafe Contributor Agreement, version 1.0, found
   in the root
    directory of this project at LICENSE, COPYING and CONTRIBUTOR respectively
   and also
    available at: http://www.maidsafe.net/licenses

    Unless required by applicable law or agreed to in writing, the MaidSafe
   Software distributed
    under the GPL Licence is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES
   OR CONDITIONS
    OF ANY KIND, either express or implied.

    See the Licences for the specific language governing permissions and
...
typedef std::set<
    NodeInfo, std::function<bool(const NodeInfo &, const NodeInfo &)> > NodeSet;
typedef std::map<NodeSet::iterator, ChildType,
                 std::function<bool(NodeSet::iterator, NodeSet::iterator)> >
MatrixMap;

struct NodeInfo {
  explicit NodeInfo(const NodeId &id_in)
      : id(id_in), matrix(new MatrixMap([id_in](NodeSet::iterator lhs,
                                                NodeSet::iterator rhs) {
  ...
  NodeInfo(NodeInfo &&other)
      : id(std::move(other.id)), matrix(std::move(other.matrix)) {}
...
NodeSet g_nodes([](const NodeInfo &lhs, const NodeInfo &rhs) {
...
void InsertNode(const MatrixRecord &matrix_record) {
  ...
  for (const auto &child : matrix_record.matrix_ids()) {
    // If matrix entry doesn't already exist in this node's matrix, add entry or
    // find entry in
...
void TakeSnapshotAndNotify() {
  static std::chrono::steady_clock::time_point last_notified(
  ...
  auto &snapshot(g_snapshots.insert(std::make_pair(
      ++g_state_id, NodeSet([](const NodeInfo & lhs, const NodeInfo & rhs) {
    return lhs.id < rhs.id;
  }))).first->second);
  ...
  while (main_itr != std::end(g_nodes)) {
    for (const auto &child : *main_itr->matrix) {
      ...
      auto g_itr(std::find_if(std::begin(snapshot), std::end(snapshot),
...
ViewableNode::ViewableNode()
    : id(), distance(), type(static_cast<ChildType>(-1)) {}
...
ViewableNode::ViewableNode(ViewableNode &&other)
    : id(std::move(other.id)), distance(std::move(other.distance)),
...
MatrixRecord::MatrixRecord()
    : owner_id_(), matrix_ids_([](const NodeId &, const NodeId &)->bool {
...
MatrixRecord::MatrixRecord(const std::string &serialised_matrix_record)
    : owner_id_(), matrix_ids_([](const NodeId &, const NodeId &) {
  return true;
}) {
  ...
  for (int i(0); i != proto_matrix_record.matrix_ids_size(); ++i) {
    AddElement(
        NodeId(proto_matrix_record.matrix_ids(i).id()),
...
std::vector<std::string> GetNodesInNetwork(int state_id) {
  ...
  for (const auto &node : snapshot_itr->second)
    hex_encoded_ids.push_back(
        node.id.ToStringEncoded(NodeId::EncodingType::kHex));

  TakeSnapshotAndNotify();
  return hex_encoded_ids;
}

std::vector<ViewableNode> GetCloseNodes(int state_id,
                                        const std::string &hex_encoded_id) {
  LOG(kInfo) << "Handling GetCloseNodes request for "
             << DebugId(NodeId(hex_encoded_id, NodeId::EncodingType::kHex))
             << " at state " << state_id;
  ...
  if (itr == std::end(snapshot_itr->second)) {
    ...
    for (const auto &node : snapshot_itr->second) {
      ...
      if (children.size() < 4) {
        ...
        if (children.size() == 4)
          needs_sorted = true;
      } else if (NodeId::CloserToTarget(
                     node.id,
                     NodeId(children[3].id, NodeId::EncodingType::kHex),
                     target_id)) {
        children[3] = ViewableNode(
            node.id.ToStringEncoded(NodeId::EncodingType::kHex),
            (target_id ^ node.id).ToStringEncoded(NodeId::EncodingType::kHex),
            ChildType::kNotConnected);
        needs_sorted = true;
      ...
      if (needs_sorted) {
        std::sort(
          ...
          return NodeId::CloserToTarget(
              NodeId(lhs.id, NodeId::EncodingType::kHex),
    ...
    for (const auto &child : *itr->matrix)
      children.emplace_back(
          ...
          (itr->id ^ (*child.first).id)
              .ToStringEncoded(NodeId::EncodingType::kHex),
          child.second);
  }
...
void Run(const std::chrono::milliseconds &notify_interval) {
  ...
  g_thread = std::move(std::thread([&]() {
    try {
      ...
      bi::message_queue matrix_messages(bi::create_only,
                                        kMessageQueueName.c_str(), 1000, 10000);
      ...
      for (;;) {
        std::string received;
        if (matrix_messages.try_receive(&input[0], 10000, received_size,
                                        priority))
          received = std::string(&input[0], received_size);
        std::unique_lock<std::mutex> lock(g_mutex);
        if (received.empty()) {
          if (g_cond_var.wait_for(lock, std::chrono::milliseconds(20), []
          {
            return g_stop;
          }))
            return;
